...
> trip current of F1. Thus, if a fault occurs downstream, **F1 cannot protect the input
> trace on the board**.
...
R6 on the ILIM pin of U1 sets the current limit. The relationship is
...
EN (pin 3) on U1 is active-high. EN connects to the input of U1, so the switch is
...
~1.18 A and then does a thermal shutdown. Thus, the error limits itself and does
not destroy parts.
...
The silkscreen shows only the signal name: `VIDEO`, `GND`, `+5V`, `AUDIO`. Thus
...
All of the back layer is a ground pour. Thus, the position of TP2 is for
convenience only. You can clip a ground lead to any B.Cu feature.

To probe the buffer and not its output, put the probe on the emitter leg of Q1.
...
**Why R7 exists.** The output of the video buffer (`/VIDEO_BUF`, the emitter of Q1
...
case, the drive of Q1 is the limit. For this reason, **R1 is 220 Ω from v2**, not
the original 330 Ω.
...
- **Q1 emitter node shorted to +5 V (breaks video).** The rail clamps the emitter of
  Q1. Thus, C2 couples +5 V, not video, into R5/J3.
...
- **VBUS input trace is undersized for its own fuse (all fabbed boards).** `/raw_5v`
  ...
  trip below 2.0 A. It is guaranteed to trip only above 3.8 A. Thus, in a 2.2–3.8 A
  ...
  name `/raw_5v`. Thus, the net got the `Default` class (0.2 mm). DRC cannot find this
...
- The symbol of D1 (`Diode:1.5KExxA`) names the two pins A1/A2 and shows no cathode.
...
- The rail clamps the emitter of Q1.
- C1 connects to the rail only through R1. Thus, the bulk capacitor does not decouple
  the rail.
...
> Thus, the B-E junction has a forward bias of 3–4 V. Q1 saturates, with its collector
...
3. Put one meter lead on +5 V, at J4 pin 3 or at the K-marked lead of D1.
...
To identify the legs of Q1 without the TO-92 orientation, measure each leg:
...
- **Emitter → J3 tip.** C2 blocks DC. Thus, this test reads open on all boards.
- **C1 + → Q1 emitter.** This test reads ~330 Ω on a good board *and* on a bad board.
  R1 is between these two points in both wirings. Thus, the test cannot identify the
  defect.

With power on, the fastest check is the DC voltage at the emitter of Q1. A correct
board shows approximately 1.5–3 V. A shorted board shows exactly the rail voltage.

**The left pad of R1 is a star point.** Three connections meet there:

- F1 (the source)
- The D1 + J4.3 branch (the load)
- The emitter of Q1

The two rail traces that come in are *collinear*. Thus, one cut severs both traces.
For this reason, the rework needs **two** jumpers, not one.

1. **Cut** the 0.8 mm trace that goes from the left pad of R1 toward the **upper
   left**. Make the cut approximately 5–8 mm from the pad.

   Do not cut the trace that comes in from the upper right. That trace is the emitter
   of Q1 and must stay.
...
4. **Make sure that R5 (75 Ω) is installed.** Measure across the pads of R5. The
   result must be ~75 Ω. R5 has no parallel path in the circuit: C2 blocks DC on one
   side, and the jack is open on the other side. Thus, this measurement is also a
   check of the meter.
...
- R1 left pad to the emitter of Q1 and to C2 pin 1: continuity
- D1 cathode to J4 pin 3 *and* to F1 pad 1: **~0 Ω**

With power on, the correct voltage at the emitter of Q1 is approximately 1.5–3 V. If
the emitter reads 5 V, the cut did not separate the traces.

## Video input biasing

The base of Q1 is DC-coupled directly to the NES video line. This board has no bias
...
directions. Thus, the pulldown is a real resistor, not a semiconductor junction. The
base of Q1 sources approximately 46 µA. That 1 kΩ resistor absorbs this current with
...
**Thus, this board has no base pulldown and no input coupling capacitor.** If you
...
**R2 (330 Ω) is not a bias resistor.** R2 is in series between J4 pin 1 and the base
of Q1. Nintendo puts a 330 Ω resistor in the same position. The base input impedance
of Q1 is ~21 kΩ, so R2 has almost no effect on the signal. R2 limits the fault
...
The most important invariant is that **the emitter of Q1 is not on any power net**.
That was the v1 fault. The output of the emitter follower connected to +5 V, with
J4.3, D1.1 and F1.1. Thus, Q1 saturated into a grounded collector, and nothing limited
...
- More clearance between the supply rails and the emitter and base nodes of Q1
...
license. Thus, it is no longer part of a hardware project.
...
board and the schematic. Before it publishes the package, it checks them again. Thus,
the package always describes the board that the gate actually verified.
...
Thus, PCBWay can do single-sided SMT, and you solder the 15 through-hole parts. Then
...
From v2, all four SMD lines (C3, R6, U1, USB-C1) are LCSC parts. Thus, PCBWay can get
...
- R2 (330 Ω): series resistor in the video input line, between J4 pin 1 and the base of Q1. See [Video input biasing](#video-input-biasing).
...
- F1 (polyfuse): a small standoff above the PCB is correct. This standoff is normal for
  ...
  **KiCad has no footprint for the Littelfuse RHEF series.** Thus, F1 uses
  ...
  Thus, that change is for v3, not a patch.

  **F1 stays a Littelfuse RHEF200 and is deliberately not an LCSC part.** F1 is
  through-hole. Thus, it is not on the SMD assembly BOM that PCBWay sources. Also, the
  ...
  - JKSEMI JK16-200T (C5183874): LCSC lists it as through-hole. But its datasheet has
    ...
    Thus, its package is not confirmed.
...
- USB-C1: GCT USB4125-GF-A-0190, SMD receptacle. Power-only, 6P, no data lines,
  ...
  **From v2, this part replaces the GCT USB4970-00-A.** LCSC does not stock the
  ...
  already used its land pattern. Thus, this change affects sourcing only, not the layout.
...
on the approved list of OSHWA. Thus, the certification applies with either license.
...
design files. Thus, it does not apply to a board fabricated from those files. Both
